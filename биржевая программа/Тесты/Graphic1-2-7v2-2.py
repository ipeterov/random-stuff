...
Directions = [-1,1]
Area_CheckMax_Values = [4,7,10,13,18,21]
Area_GiveAFuckAbouts = [1,2,3,4,6,8,10,13,15,18]
Points_UpperToBuys = [5,10,20,40]
Ms_LookToBuys = [6,8,10,13]
Points_UpToBuys = [30,40,50,70]
Ms_LookToSells = [1,2]
Points_LowerToSells = [15,20,30,45]
Points_VBuy_Adds = [0]
...
for Area_CheckMax_Value in Area_CheckMax_Values:
    for Area_GiveAFuckAbout in Area_GiveAFuckAbouts:
        file = open("alldataGraphic1-2-7v2-2.txt", "a")
            ...
            for Points_UpperToBuy in Points_UpperToBuys:
                for Points_LowerToSell in Points_LowerToSells:
                    for Points_VBuy_Add in Points_VBuy_Adds:
                        for Points_UpToBuy in Points_UpToBuys:
                            for Ms_LookToBuy in Ms_LookToBuys:
                                    ...
                                    for Direction in Directions:
                                        if Direction == 1:
                                            ...
                                            for MN in range(Area_CheckMax_Value+1, len(D) - Area_GiveAFuckAbout):
                                                ...
                                                if D[MN] >= V_MaxInArea + Points_UpperToBuy / 10000 and D[MN] - D[MN - Ms_LookToBuy] >= Points_UpToBuy/10000 and D[MN - Ms_LookToBuy] != 0:
                                            ...
                                            if my_account.money > -10 and my_account.deal_count > 0:
                                                s1 = round(-(1 - my_account.money ** (1 / my_account.deal_count)) * 10000 * 1.25,4), Area_CheckMax_Value, Area_GiveAFuckAbout, "P", Points_UpperToBuy, Direction, "b", Ms_LookToBuy, Points_UpToBuy, "s", Ms_LookToSell, Points_LowerToSell
                                            ...
                                            for MN in range(Area_CheckMax_Value+1, len(D) - Area_GiveAFuckAbout):
                                                ...
                                                if D[MN] <= V_MaxInArea - Points_UpperToBuy / 10000 and D[MN]!=0 and D[MN] - D[MN - Ms_LookToBuy] <= -Points_UpToBuy/10000:
                                                    if my_account.Buy(D[MN]):
                                                        NeedToCheckMax = 1
                                                        V_Max = D[MN]
                                                        V_Buy = D[MN]
