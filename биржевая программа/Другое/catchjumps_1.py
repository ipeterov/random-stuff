...
for minGrowth in minGrowths:
    minGrowth /= 10000
    file = open("catchjumpslog_exp.txt", "a")
        ...
        for fallCheckArea in fallCheckAreas:
            for maxFall in maxFalls:
                ...
                for MN in range(growthCheckArea, len(D)):
                    if myacc.in_deal == 0:
                        ...
                        for i in range(growthCheckArea):
                            if D[MN-i] - D[MN-i-1] > -minGrowth or D[MN-i] == 0 or D[MN-i-1] == 0:
                                wantToOpen = 0
                ...
                if myacc.deal_count != 0 and myacc.money > 0:
                    s1 = round(-(1 - myacc.money ** (1 / myacc.deal_count)) * 10000 * 1.25,4), minGrowth, growthCheckArea, fallCheckArea, maxFall, -1
