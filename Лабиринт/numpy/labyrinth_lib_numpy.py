...
class labyrinth_object:
    ...
    def __init__(self, size = (100, 100), method = 'none'):
        self.method = method
        #~ self.size = size
        #~ self.labyrinth = [cell(1,1,None)*self.size[1] for i in range(self.size[0])]
    ...
    def add_path(self, coords1, coords2):
        position = self.cells_relation(coords1, coords2)
    ...
    def remove_path(self, coords1, coords2):
        position = self.cells_relation(coords1, coords2)
    ...
    def near_passable_coords(self, coords, key = lambda x: True):
        near_coords = []
        for coord_diff in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            other_coords = [coords[0] + coord_diff[0], coords[1] + coord_diff[1]]
            if self.coords_valid(other_coords) and self.ispath(coords, other_coords) and key(other_coords):
