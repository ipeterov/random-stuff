        ...
        def collisions():
            ...
            if not self.non_empty_cells:
                for i in range(len(self.atom_list)):
                    self.grid[int(self.atom_list[i].x / diam)][int(self.atom_list[i].y / diam)].append(i)
                    self.non_empty_cells.append((int(self.atom_list[i].x / diam), int(self.atom_list[i].y / diam)))
            else:
                for x,y in self.non_empty_cells:
                    for i in self.grid[x][y]:
                        nx, ny = int(self.atom_list[i].x / diam), int(self.atom_list[i].y / diam)
                        if (x, y) != (nx, ny):
                            self.grid[x][y].remove(i)
                            self.grid[nx][ny].append(i)
                            if not self.grid[x][y]:
                                self.non_empty_cells.remove((x,y))
                            if (nx, ny) not in self.non_empty_cells:
                                self.non_empty_cells.append((nx, ny))

            for x,y in self.non_empty_cells:
                others = self.grid[x][y] + self.grid[x+1][y] + self.grid[x][y+1] + self.grid[x+1][y-1] + self.grid[x+1][y+1]
                for atom_i in self.grid[x][y]:
